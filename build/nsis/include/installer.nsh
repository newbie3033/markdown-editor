...
!include "${BUILD_RESOURCES_DIR}/nsis/include/extractAppPackage.nsh"

!ifdef APP_PACKAGE_URL
  !include "${BUILD_RESOURCES_DIR}/nsis/include/webPackage.nsh"
!endif
...
!macro installApplicationFiles
  ...
  !if "${UNINSTALLER_OUT_FILE}" != ""
    # Pre-extracted uninstaller (electron-builder's default flow; needs Wine on non-Windows hosts).
    File "/oname=${UNINSTALL_FILENAME}" "${UNINSTALLER_OUT_FILE}"
  !else
    # Write the uninstaller at install time (custom-script flow: no Wine required).
