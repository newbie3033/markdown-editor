...
/** KaTeX CSS with its preferred WOFF2 fonts embedded for portable exports. */
const katexCssWithEmbeddedWoff2 = katexCss.replace(
  /url\(fonts\/([^)]+\.woff2)\)/g,
  (source, fileName: string) => {
    const dataUrl = fontUrls[fileName]
    return dataUrl ? `url("${dataUrl}")` : source
  }
)

export const selfContainedKatexCss = katexCssWithEmbeddedWoff2.replace(
